...
with tab5:
    ...
    on thyroxine = st.radio('On thyroxine',['f','t'])
    query on thyroxine = st.radio('Query on thyroxine',['f','t'])
    on antithyroid medication = st.radio('On antithyroid medication',['f','t'])
    sick = st.radio('Sick',['f','t'])
    pregnant = st.radio('Pregnant',['f','t'])
    thyroid surgery = st.radio('Thyroid surgery',['f','t'])
    I131 treatment = st.radio('I131 treatment',['f','t'])
    query hypothyroid = st.radio('Query hypothyroid',['f','t'])
    query hyperthyroid = st.radio('Query hyperthyroid',['f','t'])
    ...
    TSH Measured = st.radio('TSH measured',['f','t'])
    TSH = st.number_input('TSH : ', -1,1000)
    T3 Measured = st.radio('T3 measured',['f','t'])    
    T3 = st.number_input('T3 : ', -1,1000)
    TT4 Measured = st.radio('TT4 measured',['f','t'])
    TT4 = st.number_input('TT4 : ', -1,1000)
    T4U Measured = st.radio('T4U measured',['f','t'])
    T4U = st.number_input('T4U : ', -1,1000)
    FTI Measured = st.radio('FTI measured',['f','t'])
    FTI = st.number_input('FTI : ', -1,1000)
    TBG Measured = st.radio('TBG measured',['f'])
    referral source = st.multiselect('Referral source',['SVHC', 'SVI', 'STMW', 'SVHD', 'Other'])
    

    features_df  = pd.DataFrame([age, sex, on thyroxine, query on thyroxine, on antithyroid medication, sick, pregnant, thyroid surgery , I131 treatment, query hypothyroid, query hyperthyroid, lithium, goitre, tumor, hypopituitary, psych, TSH Measured, TSH, T3 Measured, T3, TT4 Measured, TT4, T4U Measured, T4U, FTI Measured, FTI, TBG Measured, referral source])
